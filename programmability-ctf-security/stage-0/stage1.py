...
URLc = input("Please enter a URL:\n") #enter whatever URL
if not (URLc.startswith('//') or URLc.startswith('http://') or URLc.startswith('https://')):
    URLc = 'http://' + URLc

domain = urlparse(URLc).netloc #get the domain
...
for record in responsehis.json()['records']:
    print("Name: "+record['name'].replace('.', '(dot)'))
    print("\n")
    print("RR: "+record['rr'][:-1].replace('.', '(dot)')) #sanitize URL
    print("\n")
